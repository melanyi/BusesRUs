...
public class BusStopPlotter extends MapViewOverlay {
    ...
    public void markStops(Location currentLocation) {
        ...
        for (Stop s : StopManager.getInstance()) {
            if (Geometry.rectangleContainsPoint(northWest, southEast, s.getLocn())) {
                ...
                setNearestIcon(s, marker);
                setMarkerInfo(s, marker);
    ...
    /**
     * set the nearest stop marker to closestStopIconDrawable, else set the marker
     * to stopIconDrawable
     * @param s the Stop
     * @param marker the marker used to mark the stop
     */
    private void setNearestIcon(Stop s, Marker marker) {
    ...
    /**
     * assign all the appropriate marker information to the given marker
     * and associated stop
     *
     * @param s the Stop
     * @param marker the marker used to mark the stop
     */
    private void setMarkerInfo(Stop s, Marker marker) {
        setMarker(s, marker);
        marker.setRelatedObject(s);
        marker.setPosition(Geometry.gpFromLatLon(s.getLocn()));
        routeNumber = "";
        for (Route r : s.getRoutes()) {
            routeNumber += (r.getNumber() + "\n");
        }
    ...
    public void updateMarkerOfNearest(Stop nearest) {
        ...
        nearestStop = nearest;
        for (Stop s : stopMarkerMap.keySet()) {
            if (s.equals(nearest)) {
